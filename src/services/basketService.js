...
exports.getBaskets = async () => {
    try {
        const query = 'SELECT * FROM store.baskets';
...
exports.getBasketByUser = async (user_id) => {
    try {
        const query = 'SELECT * FROM store.baskets WHERE user_id = $1';
...
exports.createBasket = async (user_id) => {
    try {
        console.log(user_id)
        const query = 'INSERT INTO store.baskets (user_id) VALUES ($1);';
        const values = [user_id];
...
exports.addToBasket = async (product, user_id) => {
    try {
        const query = 'UPDATE store.baskets SET products = ARRAY_APPEND(products, $1) WHERE user_id = $2;';
...
exports.removeItem = async (user_id, product) => {
    try {
        const query = `UPDATE store.baskets
        SET products = (
        ...
        WHERE user_id = $1;
        `
        // 'UPDATE store.baskets SET products = ARRAY_REMOVE(products, $2) WHERE user_id = $1 LIMIT 1;'
        // AND ARRAY_POSITION(products, $2) IS NOT NULL
